...
export default function ProductCard({ product, rotate }) {
  return (
    <div className={`hang-tag p-3 flex flex-col mt-3 ${rotate}`}>
      <div className="relative">
        <img src={product.image} alt={`${product.name} - ${product.brand}`} className="w-full aspect-square object-cover border border-ink/20" />
        {product.stock <= 3 && (
      ...
      <div className="mt-3 flex-1 flex flex-col">
        ...
        <WhatsAppButton product={product} full className="mt-3 text-sm" />
      </div>
    </div>
  )
}
